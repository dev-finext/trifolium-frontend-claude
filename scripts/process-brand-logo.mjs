...
function decodePng(buf) {
    let off = 8;
    let w, h, colorType, palette = [], trns = [], idat = [];
    while (off < buf.length) {
        const len = buf.readUInt32BE(off);
        const type = buf.slice(off + 4, off + 8).toString();
        const data = buf.slice(off + 8, off + 8 + len);
        if (type === 'IHDR') { w = data.readUInt32BE(0); h = data.readUInt32BE(4); colorType = data[9]; }
        else if (type === 'PLTE') for (let i = 0; i < len; i += 3) palette.push([data[i], data[i + 1], data[i + 2]]);
        else if (type === 'tRNS') for (let i = 0; i < len; i++) trns.push(data[i]);
        else if (type === 'IDAT') idat.push(data);
        off += 12 + len;
        if (type === 'IEND') break;
    }
    const bpp = colorType === 3 ? 1 : colorType === 6 ? 4 : colorType === 2 ? 3 : null;
...
const chunk = (type, data) => { const l = Buffer.alloc(4); l.writeUInt32BE(data.length); const body = Buffer.concat([Buffer.from(type), data]); const crc = Buffer.alloc(4); crc.writeUInt32BE(crc32(body)); return Buffer.concat([l, body, crc]); };
...
function recolor(img, [tr, tg, tb]) {
    ...
    for (let o = 0; o < px.length; o += 4) {
        ...
        const ink = Math.min(1, (765 - (px[o] + px[o + 1] + px[o + 2])) / (765 - (77 + 165 + 160))); // site teal ≈ #4DA5A0
...
function resize(img, nw, nh) {
    ...
    for (let y = 0; y < nh; y++) {
        for (let x = 0; x < nw; x++) {
            ...
            const x1 = Math.floor(x * sx), x2 = Math.min(img.w, Math.ceil((x + 1) * sx));
            const y1 = Math.floor(y * sy), y2 = Math.min(img.h, Math.ceil((y + 1) * sy));
...
function iconify(mark, size, [br, bg, bb], scale = 0.66) {
    ...
    const m = resize(mark, target, Math.round(target * mark.h / mark.w));
    const px = Buffer.alloc(size * size * 4);
    for (let o = 0; o < px.length; o += 4) { px[o] = br; px[o + 1] = bg; px[o + 2] = bb; px[o + 3] = 255; }
    const ox = Math.round((size - m.w) / 2), oy = Math.round((size - m.h) / 2);
    for (let y = 0; y < m.h; y++) for (let x = 0; x < m.w; x++) {
        const so = (y * m.w + x) * 4, al = m.px[so + 3] / 255;
        if (!al) continue;
        const dо = ((oy + y) * size + (ox + x)) * 4;
        px[dо] = Math.round(m.px[so] * al + px[dо] * (1 - al));
        px[dо + 1] = Math.round(m.px[so + 1] * al + px[dо + 1] * (1 - al));
        px[dо + 2] = Math.round(m.px[so + 2] * al + px[dо + 2] * (1 - al));
    }
    return { w: size, h: size, px };
}
...
const CREAM = [0xf4, 0xf1, 0xe6];   // warm cream from the brand sheet
...
let minX = full.w, maxX = 0, minY = full.h, maxY = 0;
for (let y = 0; y < full.h; y++) for (let x = 0; x < Math.min(full.w, 660); x++) {
...
const mark = crop(full, cx, cy, Math.min(side, full.w - cx), Math.min(side, full.h - cy));

const write = (p, img) => { fs.writeFileSync(path.join(root, p), encodePng(img)); console.log(p, `${img.w}x${img.h}`, fs.statSync(path.join(root, p)).size + 'B'); };
