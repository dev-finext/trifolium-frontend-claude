...
if (!fs.existsSync(shellPath)) {
    console.error(
        'dist/preview/index.html not found — run `vite build --mode static` first.',
    );
    process.exit(1);
}
...
function writeRoute(urlPath, component, props = {}) {
    ...
    const outDir =
        urlPath === '/'
            ? distDir
            : path.join(distDir, ...urlPath.split('/').filter(Boolean));
...
const mockSrc = fs.readFileSync(
    path.join(rootDir, 'resources/js/data/mock.js'),
    'utf8',
);
const articlesBlock = mockSrc.slice(
    mockSrc.indexOf('const ARTICLES = ['),
    mockSrc.indexOf('const VIDEOS'),
);
const articleIds = [...articlesBlock.matchAll(/id:\s*'([^']+)'/g)].map(
    (m) => m[1],
);
...
console.log(
    `static demo ready: ${count} pages pre-rendered into dist/ (base: '${base || '/'}', ${articleIds.length} articles)`,
);
