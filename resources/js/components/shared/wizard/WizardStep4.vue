<script setup>
// Wizard Step 4 — Instructions / Directions. Two cards:
//   • הוראות לבית המרקחת — internal, shown first (not exposed to the patient).
//   • הנחיות למטופל/ת — customer-facing zone (accent-framed) that holds the
//     daily-dose block (מינון יומי) + extra patient directions.
// Nothing in the dose block is pre-selected — the practitioner picks each value.
// Embedded into step 3's directions slot (nav hidden).
...
<template>
    <div class="col gap-[16px]">
        <!-- ─── Card: הוראות לבית המרקחת (internal — above the patient area) ── -->
        ...
        <!-- ─── Card: הנחיות למטופל/ת — CUSTOMER-FACING zone. Everything here
             reaches the patient (daily dose + printed directions), so the card
             is visually flagged as such: accent frame + header badge. ── -->
        <section class="card overflow-hidden border-[1.5px] border-accent p-0">
            ...
            <div class="col gap-[18px] px-[20px] pt-[16px] pb-[20px]">
                <!-- מינון יומי -->
                <div>
                    <div
                    ...
                    <div
                        class="overflow-hidden rounded-control border border-line"
                    >
                        <div class="dose-grid grid grid-cols-[1fr_1fr_1fr_2fr]">
                            ...
                            <DoseCol icon="drop" label="יחידת מידה">
                                <select
                                    ...
                                    :value="formula.dose?.unit || ''"
                                    @change="
                                        setDose({ unit: $event.target.value })
                                    "
                                >
                                    <option value="" disabled>אנא בחר/י</option>
                                    <option
                                        v-for="u in unitIds"
                                        :key="u"
                                        :value="u"
