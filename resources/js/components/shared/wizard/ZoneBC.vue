...
import IngredientSearchBar from '@/components/shared/wizard/IngredientSearchBar.vue';
import PresetFormulaPicker from '@/components/shared/wizard/PresetFormulaPicker.vue';
import SelectedColumn from '@/components/shared/wizard/SelectedColumn.vue';
import {
    reflowParts,
    isHerbCompatible,
    herbMatchesQuery,
...
const presetBase = computed(() => {
    if (!props.formula.presetBaseId) {
        return null;
    }

    return (
        (PRESET_FORMULAS || []).find(
            (p) => p.id === props.formula.presetBaseId,
        ) ||
        (FREEPOUR_FORMULAS || []).find(
            (p) => p.id === props.formula.presetBaseId,
        ) ||
        null
    );
...
const inFormula = (id) =>
    props.formula.ingredients.find((i) => i.herbId === id);

function add(herb) {
    if (inFormula(herb.id)) {
        return;
    }

    let next = [
        ...props.formula.ingredients,
        { herbId: herb.id, pct: 0, qty: 0, parts: '', preset: false },
    ];
...
function loadPreset(preset) {
    ...
    props.setF({
        ingredients: reflowParts(
            [...presetIngredients, ...nonPreset],
            volume.value || 0,
        ),
...
function clearPreset() {
    const nonPreset = props.formula.ingredients.filter((i) => !i.preset);
    props.setF({
        ingredients: reflowParts(nonPreset, volume.value || 0),
        presetBaseId: null,
    });
}
...
function update({ idx, value }) {
    if (
        props.formula.ingredients[idx] &&
        props.formula.ingredients[idx].preset
    ) {
        return;
    }

    const v = value === '' ? '' : Math.max(0, +value || 0);
    let next = props.formula.ingredients.map((row, i) =>
        i === idx ? { ...row, parts: v } : row,
    );
    next = reflowParts(next, volume.value || 0);
    props.setF({ ingredients: next });
}
...
const filtered = computed(() =>
    sortHerbsByQuery(
        HERBS.filter(
            (h) =>
                isHerbCompatible(h, props.formula.typeId) &&
                herbMatchesQuery(h, search.value, {
                    allowPinyin: isChinese.value,
                }),
        ),
        search.value,
        isChinese.value,
    ...
    props.formula.ingredients.forEach((row) => {
        findInteractions(row.herbId, props.patientMeds).forEach((k) => {
            ...
            alerts.push({
                herb,
                k,
                drug: props.patientMeds.match(k.drug)?.[0] || 'תרופה',
            });
        });
    });
...
function onPickEditable(f) {
    if (props.loadSavedFormula) {
        props.loadSavedFormula(f);
    }
...
<template>
    <section class="relative" id="tf-anchor-ingredients">
        ...
        <div
            ...
            role="alert"
            class="mt-[16px] flex items-start gap-[11px] rounded-card border border-[#e6c3ba] bg-[#faf0ed] px-[16px] py-[12px] text-danger"
        >
            <span
                class="mt-[1px] inline-flex h-[22px] w-[22px] shrink-0 items-center justify-center rounded-[50%] bg-danger text-white"
            >
                <svg
                    width="13"
                    height="13"
            ...
            <div class="min-w-0">
                <div class="text-[13.5px] leading-[1.45] font-semibold">
                    אינטראקציה אפשרית — המשך באחריות המטפל
                </div>
                <div
                    v-for="(a, i) in interactionAlerts.slice(0, 2)"
                    :key="i"
                    class="mt-[4px] text-[12.5px] leading-[1.5] font-medium text-ink-2"
                >
                    <strong>{{ a.herb.lat }}</strong> +
                    <strong>{{ a.drug }}</strong> — {{ a.k.note }}
                </div>
            </div>
        </div>
