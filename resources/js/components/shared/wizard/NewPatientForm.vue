...
import MedicationSearchPicker from '@/components/shared/wizard/MedicationSearchPicker.vue';
import SegmentedControl from '@/components/shared/wizard/SegmentedControl.vue';
import YesNoToggle from '@/components/shared/wizard/YesNoToggle.vue';
...
const phoneError = computed(
    () => (props.value.phone || '').trim() !== '' && !phoneValid.value,
);
...
function onAgeInput(e) {
    ...
    if (v !== '') {
        ...
        if (isNaN(n)) {
            ...
            v = String(Math.max(0, Math.min(120, n)));
        }
    }
    ...
    ageError.value =
        raw !== '' && (isNaN(rawNum) || rawNum < 0 || rawNum > 120);
    const ageNum = Number(v);
    const inRepro =
        props.value.sex === 'female' &&
        v !== '' &&
        ageNum >= 15 &&
        ageNum <= 60;
...
<template>
    <div class="col gap-[16px]">
        ...
        <div class="grid grid-cols-[1fr_1fr] gap-[16px]">
            <Field label="שם פרטי *">
                <input
                    ...
                    placeholder="יוסי"
                    @input="set('firstName', $event.target.value)"
                />
            </Field>
            <Field label="שם משפחה *">
                <input
                    ...
                    placeholder="כהן"
                    @input="set('lastName', $event.target.value)"
                />
            </Field>
        </div>
        ...
        <Field label="טלפון *">
            <div class="input-wrap">
                ...
                <input
                    ...
                    :aria-describedby="
                        phoneError ? 'np-phone-error' : undefined
                    "
            ...
            <div
                ...
                id="np-phone-error"
                role="alert"
                class="small mt-[8px] font-semibold text-danger"
            >
                מספר טלפון לא תקין — יש להזין מספר בן 9–10 ספרות (לדוגמה:
                050-1234567).
            </div>
        </Field>
        ...
        <div class="flex flex-wrap items-start gap-[28px]">
            <Field label="גיל *">
                <div class="flex items-center gap-[12px]">
                    ...
                    />
                    <label
                        class="flex cursor-pointer items-center gap-[8px] text-[13px] whitespace-nowrap text-ink-2"
                    >
                        <input
                            type="checkbox"
                            :checked="value.ageEstimated === true"
                            class="h-[16px] w-[16px] cursor-pointer accent-(--ink)"
                ...
                <div
                    ...
                    class="small mt-[8px] font-semibold text-danger"
                >
                    יש להזין גיל בין <span class="num">0</span> ל־<span
                        class="num"
                        >120</span
                    >.
                </div>
            ...
            <Field label="נוטל/ת תרופות *">
                <YesNoToggle
                    :model-value="value.onMeds"
                    @update:model-value="onMedsChange"
                />
            </Field>
        </div>
        ...
        >
            <Field label="בהריון *">
                <YesNoToggle
                    :model-value="value.pregnant"
                    @update:model-value="set('pregnant', $event)"
                />
            </Field>
            <Field label="האם מניקה? *">
                <YesNoToggle
                    :model-value="value.breastfeeding"
                    @update:model-value="set('breastfeeding', $event)"
                />
            </Field>
        </template>
        ...
        <div>
            <div v-if="value.onMeds === true" class="col mt-[20px] gap-[12px]">
                <MedsSafetyDeclaration
                    :patient-meds="
                        Array.isArray(value.meds)
                            ? value.meds.join(', ')
                            : value.meds || ''
                    "
                    confirmable
