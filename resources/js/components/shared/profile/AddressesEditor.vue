...
import AddressCardEditor from '@/components/shared/profile/AddressCardEditor.vue';
...
const update = (id, next) =>
    emit(
        'update:list',
        props.list.map((a) => (a.id === id ? next : a)),
    );
const makePrimary = (id) =>
    emit(
        'update:list',
        props.list.map((a) => ({ ...a, primary: a.id === id })),
    );
const remove = (id) => {
    const removingPrimary = props.list.find((a) => a.id === id)?.primary;
    const next = props.list.filter((a) => a.id !== id);

    if (removingPrimary && next.length && !next.some((a) => a.primary)) {
