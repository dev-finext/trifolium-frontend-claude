...
// ---- validation ----
// Kept verbatim from the prototype even though advancing no longer calls them
// (see the note on advance()); the backend port will want these rules.
...
// Validation removed from the registration forms — advancing and submitting
// always proceed; no inline errors, no required gating.
        ...
        <main :style="{ maxWidth: '760px', margin: '0 auto', padding: '36px 24px 96px' }">
            <div :style="{ display: 'flex', flexDirection: 'column', gap: '26px' }">
                ...
                <div v-if="active === 'personal'" :style="{ display: 'flex', flexDirection: 'column', gap: '24px' }">
                    <div :style="gridStyle">
                        <AuthInput label="שם פרטי" icon="user" :model-value="p.first" :error="err.first || ''" @update:model-value="(v) => { p.first = v; clr('first'); }" />
                        <AuthInput label="שם משפחה" icon="user" :model-value="p.last" :error="err.last || ''" @update:model-value="(v) => { p.last = v; clr('last'); }" />
                        <AuthInput label="תאריך לידה" icon="calendar" type="date" dir="ltr" :model-value="p.dob" :error="err.dob || ''" @update:model-value="(v) => { p.dob = v; clr('dob'); }" />
                        <AuthInput label="מספר זהות" icon="user" numeric placeholder="9 ספרות" input-mode="numeric" :max-length="9" :model-value="p.id" :error="err.id || ''" @update:model-value="(v) => { p.id = onlyDigits(v); clr('id'); }" />
                        <AuthInput label="כתובת דוא״ל" icon="mail" type="email" placeholder="name@example.com" :model-value="p.email" :error="err.email || ''" @update:model-value="(v) => { p.email = v; clr('email'); }" />
                        <AuthInput label="טלפון" icon="phone" numeric placeholder="050-0000000" input-mode="tel" :model-value="p.phone" :error="err.phone || ''" @update:model-value="(v) => { p.phone = v; clr('phone'); }" />
                    </div>
                ...
                <div v-else-if="active === 'address'" :style="{ display: 'flex', flexDirection: 'column', gap: '24px' }">
                    <div :style="gridStyle">
                        ...
                        <AuthInput label="מספר בית" numeric input-mode="numeric" :model-value="a.house" :error="err.house || ''" @update:model-value="(v) => { a.house = v; clr('house'); }" />
                        <AuthInput v-model="a.apt" label="דירה" numeric input-mode="numeric" />
                        <AuthInput v-model="a.floor" label="קומה" numeric input-mode="numeric" />
                        <AuthInput v-model="a.entrance" label="כניסה" />
                    </div>
                    <div :style="{ display: 'flex', justifyContent: 'flex-start' }">
                ...
                <div v-else-if="active === 'professional'" :style="{ display: 'flex', flexDirection: 'column', gap: '24px' }">
                    <!-- role -->
                    <div>
                        <label class="field-label" style="margin-bottom: 10px">אני:</label>
                        <RoleRadio
                            :model-value="role"
                    ...
                    <div v-if="role === 'student'" :style="{ display: 'flex', flexDirection: 'column', gap: '24px' }">
                        <div :style="gridStyle">
                            <AuthInput label="שם המכללה" :model-value="stu.college" :error="err.college || ''" @update:model-value="(v) => { stu.college = v; clr('college'); }" />
                            <AuthInput v-model="stu.teacher" label="שם המורה" />
                            <AuthInput v-model="stu.head" label="שם מנהל/ת המגמה" />
                    ...
                    <div v-if="role === 'therapist'" :style="{ display: 'flex', flexDirection: 'column', gap: '24px' }">
                        <div :style="gridStyle">
                            <AuthInput label="שנת סיום הלימודים" numeric placeholder="2026" input-mode="numeric" :max-length="4" :model-value="th.gradYear" :error="err.gradYear || ''" @update:model-value="(v) => { th.gradYear = onlyDigits(v); clr('gradYear'); }" />
                        ...
                        >
                            ...
                            <AuthInput label="שם הקליניקה" icon="home" :model-value="th.clinicName" :error="err.clinicName || ''" @update:model-value="(v) => { th.clinicName = v; clr('clinicName'); }" />
                            <div>
                                <label class="field-label">תיאור העבודה שלי / תיאור הקליניקה שלי</label>
                                <textarea
